...
export default function Home() {
  ...
  return (
    <>
      ...
      {cart.length > 0 && (
        <div className="floating-checkout" style={{ display: "flex" }}>
          <div className="total-v">
            <div id="loyalty-msg" style={{ fontSize: "0.65rem", color: "var(--primary)", fontWeight: "700", marginBottom: "2px" }}>
              {potentialPoints > 0 ? `+${potentialPoints} Poin Pilihan` : ""}
            </div>
            <div style={{ fontSize: "0.75rem", opacity: "0.8", fontWeight: "600" }}>Total + Ongkir</div>
            <div id="total-val" style={{ fontSize: "1.1rem", fontWeight: "700" }}>
              Rp {subtotal.toLocaleString('id-ID')}
            </div>
          </div>
          <button className="checkout-btn" onClick={() => window.dispatchEvent(new CustomEvent('open-cart'))}>
            Buka Keranjang <i className="fa-solid fa-arrow-right" style={{ marginLeft: "8px" }}></i>
          </button>
        </div>
      )}

      <footer style={{ textAlign: "center", padding: "40px 20px", color: "var(--text-muted)", fontSize: "0.8rem", borderTop: "1px solid #f0f0f0", marginTop: "50px" }}>
        <p>&copy; 2024 La Misha Bakehouse Metro. All Rights Reserved.</p>
      </footer>
